/*
 * Copyright (C) 2025.  Tim Frey, Christian Schmitt
 *
 * Licensed under the OPEN COMPENSATION TOKEN LICENSE (the "License").
 * You may not use this file except in compliance with the License.
 * You may obtain a copy of the License at <a href="https://www.license-token.com/license/text">https://www.license-token.com/license/text</a>
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either expressed or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 * @octl.sid: 4efebf98-4efe-ff98-bf98-00004eb04127
 *
 */

package com.iunera.jsonldjava.schemaorg.metadatatypes;

import com.iunera.jsonldjava.schemaorg.associations.JsonldProperty;
import com.iunera.jsonldjava.schemaorg.associations.JsonldType;
import com.iunera.jsonldjava.schemaorg.associations.Vertex;

import java.util.List;

/**
 * A single message from a sender to one or more organizations or people.
 */
@Vertex
@JsonldType("http://schema.org/Message")
public class Message extends CreativeWork {
    /**
     * The date/time at which the message was sent.
     */
    @JsonldProperty("http://schema.org/dateSent")
    public java.util.Date dateSent;

    /**
     * A sub property of participant. The participant who is at the sending end of the action.
     */
    @JsonldProperty("http://schema.org/sender")
    public List<Audience> senderAudience;

    /**
     * A sub property of participant. The participant who is at the receiving end of the action.
     */
    @JsonldProperty("http://schema.org/recipient")
    public List<Person> recipientPerson;

    /**
     * The date/time the message was received if a single recipient exists.
     */
    @JsonldProperty("http://schema.org/dateReceived")
    public java.util.Date dateReceived;

    /**
     * A sub property of participant. The participant who is at the receiving end of the action.
     */
    @JsonldProperty("http://schema.org/recipient")
    public List<Audience> recipientAudience;

    /**
     * A sub property of participant. The participant who is at the sending end of the action.
     */
    @JsonldProperty("http://schema.org/sender")
    public List<Organization> senderOrganization;

    /**
     * The date/time at which the message has been read by the recipient if a single recipient exists.
     */
    @JsonldProperty("http://schema.org/dateRead")
    public java.util.Date dateRead;

    /**
     * A sub property of participant. The participant who is at the receiving end of the action.
     */
    @JsonldProperty("http://schema.org/recipient")
    public List<Organization> recipientOrganization;

    /**
     * A sub property of participant. The participant who is at the sending end of the action.
     */
    @JsonldProperty("http://schema.org/sender")
    public List<Person> senderPerson;

    /**
     * A CreativeWork attached to the message.
     */
    @JsonldProperty("http://schema.org/messageAttachment")
    public CreativeWork messageAttachment;
}
